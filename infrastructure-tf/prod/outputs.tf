output "aws_account_id" {
  description = "AWS account where the production resources are managed."
...
output "nat_gateway_public_ip" {
  description = "NAT Gateway public IP when explicitly enabled; null in cost-optimized production."
...
output "database_vpc_peering_connection_id" {
  description = "Peering connection between the production VPC and the existing database VPC."
  value       = module.network.database_vpc_peering_connection_id
}

output "database_private_hostname" {
  description = "Set this value as POSTGRES_HOST in the production backend secret after apply."
...
output "github_actions_role_arns" {
  description = "GitHub OIDC roles for image promotion and role-specific deployment."
  value = merge(
    { promote = module.cicd.github_promote_role_arn },
...
output "dns_records" {
  description = "Desired public DNS records reconciled by prod/dns/sync-records.sh through the adm.tools API."
