...
apply  Create or update only records below eclipcity.digitee.space.
...
MANAGED_DOMAIN_NAME="${MANAGED_DOMAIN_NAME:-eclipcity.digitee.space}"
...
if [[ -n "${DNS_RECORDS_FILE:-}" ]]; then
  ...
  desired_records="$(terraform -chdir="${PROD_DIR}" output -json dns_records)"
fi
